...
- **Change engine** — semantic diff (what actually changed) and migration planning (the SQL to get from one state to another).
...
3. **Object identity in diff** — deciding when two things across states are the same object (renames, moves, retypes) before judging what changed.
